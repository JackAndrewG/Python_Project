...
function escribirdias() {
    ...
    for (i = 1; i < 7; i++) { //localizar fila
        ...
        for (j = 0; j < 7; j++) {
            ...
            var contenido = '<input type="hidden" id= "anio' + id + '" value="' + mianno + '">';
            contenido += '<input type="hidden" id= "mes' + id + '" value="' + (mimes+1) + '">';
            contenido += '<input type="hidden" id= "dia' + id + '" value="' + midia + '">' + midia;
            celda.innerHTML = contenido;
            //domingos en rojo
            if (j === 6) {
                celda.style.color = "#A93226";
            }
            ...
            if (mimes === meshoy && midia === diahoy && mianno === annohoy) {
                celda.style.backgroundColor = "#85C1E9";
                celda.style.background = '-moz-linear-gradient(top, #b32b0c, #cd310d)';
                celda.innerHTML = '<cite title="Fecha Actual">' + contenido + '</cite>';
            }
...
function mifecha() {
   ...
   function formatoFecha(anio, mes, dia) {
    ...
    function fechaValida(anioSelect, mesSelect, diaSelect) {
        var validacion = false;
        var anio = new Date().getFullYear();
        var mes = new Date().getMonth() + 1;
        var dia = new Date().getDate();
        if (anioSelect <= anio && mesSelect >= mes) {
            if (diaSelect >= dia) {
                validacion = true;
